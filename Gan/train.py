    ...
    def train_step(self, batch, accumulate_steps=1):
        ...
        if self.scaler:
            with torch.amp.autocast('cuda'):
                ...
                # Discriminator loss
                d_loss, d_components = self.criterion.discriminator_loss(
                    real_pred, fake_pred)
        ...
        if self.scaler:
            with torch.amp.autocast('cuda'):
                ...
                # Generator loss
                g_loss, g_components = self.criterion.generator_loss(
                    fake_pred, fake_masks, real_masks)
    ...
    def validate(self, val_loader):
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                g_loss, _ = self.criterion.generator_loss(
                    fake_pred, fake_masks, real_masks)
                d_loss, _ = self.criterion.discriminator_loss(
                    real_pred, fake_pred)
        ...
        for epoch in range(self.start_epoch, self.config['num_epochs']):
            ...
            for batch_idx, batch in enumerate(train_loader):
                step_results = self.train_step(
                    batch, self.config['accumulate_steps'])
                ...
                if self.step_count % self.config['log_interval'] == 0:
                    self.writer.add_scalar(
                        'Train/G_Loss', step_results['g_loss'], self.step_count)
                    self.writer.add_scalar(
                        'Train/D_Loss', step_results['d_loss'], self.step_count)
                    self.writer.add_scalar(
                        'Train/IoU', step_results['metrics']['iou'], self.step_count)
                    self.writer.add_scalar(
                        'Train/Dice',
                        step_results['metrics']['dice'],
                        self.step_count)
                ...
                if batch_idx % self.config['print_interval'] == 0:
                    ...
                    print(
                        f"  G Loss: {
                            step_results['g_loss']:.4f}, D Loss: {
                            step_results['d_loss']:.4f}")
                    print(
                        f"  IoU: {
                            step_results['metrics']['iou']:.4f}, Dice: {
                            step_results['metrics']['dice']:.4f}")
            ...
            avg_train_losses = {k: sum(v) / len(v)
                                for k, v in epoch_losses.items()}
            avg_train_metrics = {k: sum(v) / len(v)
            ...
            print(
                f"Train - G Loss: {
                    avg_train_losses['g_loss']:.4f}, D Loss: {
                    avg_train_losses['d_loss']:.4f}")
            print(
                f"Train - IoU: {
                    avg_train_metrics['iou']:.4f}, Dice: {
                    avg_train_metrics['dice']:.4f}")
            print(
                f"Val   - G Loss: {val_losses['g_loss']:.4f}, D Loss: {val_losses['d_loss']:.4f}")
            print(
            ...
            self.writer.add_scalar(
                'Epoch/Train_G_Loss',
                avg_train_losses['g_loss'],
                epoch)
            self.writer.add_scalar(
                'Epoch/Train_D_Loss',
                avg_train_losses['d_loss'],
                epoch)
            self.writer.add_scalar(
                'Epoch/Val_G_Loss', val_losses['g_loss'], epoch)
            self.writer.add_scalar(
                'Epoch/Val_D_Loss', val_losses['d_loss'], epoch)
            self.writer.add_scalar('Epoch/Val_IoU', val_metrics['iou'], epoch)
            self.writer.add_scalar(
                'Epoch/Val_Dice', val_metrics['dice'], epoch)
